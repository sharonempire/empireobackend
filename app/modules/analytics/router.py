...
async def get_dashboard_summary(
    user_id: UUID | None = Query(None, description="Filter stats for a specific user"),
    role: str | None = Query(None, description="Filter stats by role (e.g. counselor, processor)"),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """Combined dashboard stats: leads + attendance summary."""
    return await service.dashboard_summary(db, user_id=user_id, role=role)
